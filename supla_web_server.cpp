...
const char * Supported_ActivateServer[2] = {
  "Nie aktywna",
  "Aktywna"
};
...
String supla_webpage_start(int save) {
  ...
  if (nr_ds18b20 > 0) {
    ...
    for (int i = 0; i < nr_ds18b20; i++) {
      long temp_long = round(get_temperature(ds18b20_channel[i].channel, 0));
      double temp = get_temperature(ds18b20_channel[i].channel, 0);
      if (ds18b20_channel[i].type == 1) {
        ...
        content += "</label></i>";
             ...
             content += "<i><input name='supla_light_sensor' value='" + String(temp_long) + "' readonly><label style='left: calc(100% - 255px)'>lux</label>";
             content += "<label>";
             content += "Natężenie";
             content += "</label></i>"; 
      }
  ...
  if (nr_bme > 0 ) {
    ...
    int pressure = round(bme_channel.pressure);
    int pressure_sea = round(bme_channel.pressure_sea);

    content += "<i><input name='supla_bme280_temp' value='" + String(temp_html) + "' readonly><label style='left: calc(100% - 255px)'>&#x2103</label>";
    content += "<label>";
    content += "Temperatura";
    content += "</label></i>"; 
...
// czujnik sht31d 
    content += "<div class='w'>";
    content += "<h3>SHT31 - HECA </h3>";
    get_temperature_and_humidity(sht_channel.temperature_channel, &temp_html, &humidity_html);
...
//  CZUJNIK SDS011
    ...
    content += "<div class='w'>";
    content += "<h3>SDS011</h3>";

    content += "<i><input name='SDS011_CWP' value='" + String(CustomWorkingPeriod) + "' ><label style='left: calc(100% - 255px)'>minut</label>";
    content += "<label>";
    content += "Czasookres pracy";
    content += "</label></i>"; 

    content += "<i><input name='SDS011_NORM25' value='" + String(norma_pm25) + "' ><label style='left: calc(100% - 255px)'>μg/m³</label>";
    content += "<label>";
    content += "Norma dla PM 2.5";
    content += "</label></i>"; 
    
    content += "<i><input name='SDS011_NORM10' value='" + String(norma_pm10) + "' ><label style='left: calc(100% - 255px)'>μg/m³</label>";
    content += "<label>";
  ...
  // AQI.ECO
    ...
    content += ">Komunikacja ";
    content += "</label>";
    content += "<select name='aqieco_set'>";
    for (int suported_monitor = 0; suported_monitor < 2; suported_monitor++) {
        content += "<option value='";
        content += suported_monitor;
        char select_monitor_state = read_aqi_eco_state();
        if (select_monitor_state == suported_monitor) {
            content += "' selected>";
          }
          else content += "' >";
          content += (Supported_ActivateServer[suported_monitor]);
    }        
    content += "</select></i>"; 
    content += "<i>";
    
    content += "<input class='text' type='text' name='aqi_eco_host' value='" + String(read_aqi_eco_host().c_str()) +   "'>";    //api.aqi.eco'>";
    content += "<label>AQI.ECO HOST</label>";
    content += "</i>";
    content += "<i>";
    content += "<input class='text' type='text' name='aqi_eco_path' value='" + String(read_aqi_eco_path().c_str()) + "'>";      // update/f92fa5567b72d38c092209378a03cd54'>";
    content += "<label>AQI.ECO PATH</label><br>";
    content += "</i>";
    content += "</div>";

    content += "<div class='w'>";
    content += "<h3>Serwer AIR MONITOR</h3>";
  ...
  // AIR.MONITOR
    ...
    content += ">Komunikacja ";
    content += "</label>";
    content += "<select name='air_monitor_set'>";
    for (int suported_monitor = 0; suported_monitor < 2; suported_monitor++) {
        content += "<option value='";
        content += suported_monitor;
        char select_monitor_state = read_air_monitor_state(); // 
        if (select_monitor_state == suported_monitor) {
            content += "' selected>";
          }
          else content += "' >";
    ...
    content += "<i>";
    content += "<input class='text' type='text' name='airMonitorServer' value='" + String(read_air_monitor_server().c_str()) + "'>";  // api.airmonitor.pl'>";
    content += "<label>airMonitor HOST</label>";
    content += "</i>";
    
    content += "<i>";
    content += "<input class='text' type='text' name='airMonitor_latitude' value='" + String(read_air_monitor_latitude().c_str()) + "'>";  // 50.084449'>";
    content += "<label>Szer. geogr.</label>";
    content += "</i>";
    content += "<i>";
    
    content += "<input class='text' type='text' name='airMonitor_longitude' value='" + String(read_air_monitor_longitude().c_str()) + "'>"; // 18.832895'>";
    content += "<label>Długość geogr.</label>";
    content += "</i>";     
    content += "</div>";
