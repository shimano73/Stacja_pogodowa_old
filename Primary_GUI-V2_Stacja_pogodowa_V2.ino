...
char LATITUDE[10]  = "50.xxxxxx";
char LONGITUDE[10] = "18.xxxxxx";
...
char AQI_ECO_PATH[50] = "/update/xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
bool DEBUG = true;
...
double  speed_wind;
// double One_rpm_speed = 2.40 ; // Wiatromierz skalibrowany tak że 1 impuls na sekundę oznacza prękość wiatru 2.4km/h
double One_rpm_speed = 0.666666; // Wiatromierz skalibrowany tak że 1 impuls na sekundę oznacza prękość wiatru m/s

double new_speed_wind;
...
void setup() {
  ...
  strcpy(AQI_ECO_PATH ,read_aqi_eco_path().c_str());
 
  strcpy(LATITUDE ,read_air_monitor_latitude().c_str());
  strcpy(LONGITUDE ,read_air_monitor_longitude().c_str());
  
  String airMonitorServerName1 = String(read_air_monitor_server().c_str());
  airMonitorServerName = strcpy((char*)malloc(airMonitorServerName1.length() + 1), airMonitorServerName1.c_str());

  AIRMONITOR_ON = read_aqi_eco_state();
  AIRMONITOR_ON = read_air_monitor_state();
...
void supla_timer() {
    ...
    if (millis() - lastmillis >= 1000) {
        ...
        if (rpm>0) {
              new_speed_wind = One_rpm_speed *(rpm /60); // 
        ...
        //pomiar_wiatru[0]=predkosc_ms; 
        pomiar_wiatru[0]=new_speed_wind; 
        licz_pomiar++;    
...
void createWebServer() {
  ...
  httpServer.on("/set0", []() {
    ...
    save_aqi_eco_state(httpServer.arg("aqieco_set")); 
    save_aqi_eco_host(httpServer.arg("aqi_eco_host"));
    save_aqi_eco_path(httpServer.arg("aqi_eco_path"));
    save_air_monitor_state(httpServer.arg("air_monitor_set"));
    save_air_monitor_server(httpServer.arg("airMonitorServer"));
    save_air_monitor_latitude(httpServer.arg("airMonitor_latitude"));
    save_air_monitor_longitude(httpServer.arg("airMonitor_longitude"));

    Serial.print("Aqi.eco : ");
    Serial.println(read_aqi_eco_state());
    Serial.print("Air_monitor.pl : ");
    Serial.println(read_air_monitor_state());
...
double get_pressure(int channelNumber, double last_val) {
  ...
  pressure = round(bme.readPressure());
  pressure_sea = round(pressure / pow(2.718281828, - (bme_channel.elevation / ((273.15 + bme_channel.temp) * 29.263))) / 100.0F);
...
double get_temperature(int channelNumber, double last_val) {
  ...
  switch (channelNumber)
        ...
        case 5:
         t = round( myBH1750.readLightLevel());
...
void supla_heca_start(void) {
  if (!initHECA()) Serial.println("Nie znaleleziono modułu HECA, sprawdz poprawność podłączenia i okablowanie!");        
}
