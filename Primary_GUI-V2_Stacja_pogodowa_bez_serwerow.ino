...
double predkosc_mph; //Utworzenie zmiennej mile/godzinÄ
...
void setup() {
  ...
  supla_dht_start();  //
  supla_I2C_start();  // 
  supla_bme_start();  // czujnik temperatury i wilgotności zewnętrznej 
  supla_heca_start(); // heca - "podgrzewacz" powietrza zasysanego przez sds011
  supla_sensor_light_start();

  SuplaDevice.addWindSensor(); // czunjnik prędkości wiatru
  SuplaDevice.addRainSensor(); // czujnik odpadów deszczu

  add_Relay(100); // Przełącznik / "resetownik" temperatury min 
  add_Relay(101); // Przełącznik / "resetownik" temperatury max 
...
void supla_timer() {
    ...
    if (millis() - lastmillis >= 1000) {
        ...
        if (rpm>0) {
            predkosc_mph = diameter / 12 * 3.14 * rpm * 60 / 5280;//Odczyt prÄdkoĹci wiatru w milach/godzinÄ
            predkosc_mph = predkosc_mph * 3.5; // Kalibracja bĹÄdu odczytu, wartoĹÄ naleĹźy dobraÄ we wĹasnym zakresie
            predkosc_kmh = predkosc_mph * 1.609;// Zamiana mil/godzinÄ na km/h
   ...
   switch (channelNumber)
        { 
         ...
         case 2:         
                ...
                tab_measurement[0] = bme.readTemperature(); // bez wyliczania uśrednionego pomiaru 
